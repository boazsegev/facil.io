...
static void on_http_request(http_s *h) {
  ...
  FIOBJ req = http2str(h);
  fio_str_info_s i = fiobj_str_info(req);
  http_set_header(h, HTTP_HEADER_CONTENT_TYPE, http_mimetype_find("txt", 3));
  http_send_body(h, i.buf, i.len);
  fprintf(stderr,
          "Response (%s)\n%s\n",
          fiobj_str_info(HTTP_HEADER_CONTENT_TYPE).buf,
          i.buf);
  fiobj_free(req);
}
...
void initialize_cli(int argc, char const *argv[]) {
  ...
  fio_cli_start(
      ...
      // WebSocket Settings arguments
      FIO_CLI_PRINT_HEADER("Other Settings:"),
      FIO_CLI_BOOL("-debug -V renable DEBUG mode logging."));
