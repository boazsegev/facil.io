...
void initialize_cli(int argc, char const *argv[]) {
  ...
  fio_cli_start(
      ...
      FIO_CLI_TYPE_INT, "-port -p port number to listen to. defaults port 3000",
      FIO_CLI_TYPE_INT, "-workers -w number of processes to use.",
      FIO_CLI_TYPE_INT, "-threads -t number of threads per process.",
      FIO_CLI_TYPE_INT, "-log -v request verbosity (logging).",
      FIO_CLI_TYPE_BOOL, "-public -www public folder, for static file service.",
