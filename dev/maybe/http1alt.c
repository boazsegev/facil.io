...
static void http1_on_data(intptr_t uuid, http1_protocol_s *pr) {
  ...
  if (request->request.body_file <= 0) {
    ...
    //     fprintf(stderr, "%c", request->buffer[i] ? request->buffer[i] : '-');
    //   }
    //   fprintf(stderr, "\n");
    // } else
    //   fprintf(stderr,
    ...
    //           " current len == %lu"
    //           " request->buffer_pos == %lu\n",
    //           pr->len, request->buffer_pos);
...
*/
protocol_s *http11_on_open(intptr_t fd, http_settings_s *settings) {
