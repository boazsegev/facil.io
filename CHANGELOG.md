...
My vision for 0.8.0 was to make these tools available to more developers, which meant it was time to separate the IO core library (`fio.h` and `fio.c`) from the non-IO core library, unify the API and remove some code duplication...

... the result was the swiss-army-knife beauty of `fio-stl.h`.

I though about it as a small standard-library extension with a minimal STL, much like C++ has, only for C, and moved the best non-network features facil.io offers into this single library, the facil.io Simple Template Library, `fio-stl.h`.
