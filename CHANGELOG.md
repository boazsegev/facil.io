...
Previous versions had the IO core library feature-set spread across multiple files. The single-file library approach (actually 2 files, a header and a C file), was first implemented in the 0.7.x version family. The 3 library approach to the IO core (`fio-stl.h`, `fio.h`, `fio.c`) starts with the 0.8.x version family. 
