...
static struct {
  ...
  void *udata1;
    ...
    {CB_ON_START, (void *)0},
    {CB_ON_ARG, (void *)1},
    {CB_ON_ARG_UNESCAPE, (void *)0},
    {CB_ON_ARG_UNESCAPE, (void *)0},
    {CB_ON_TEST, (void *)0},
    {CB_ON_START, (void *)0},
    {CB_ON_ARG_UNESCAPE, (void *)1},
    {CB_ON_ARG_UNESCAPE, (void *)1},
    {CB_ON_TEST, (void *)1},
    {CB_ERROR, NULL},
};
...
static void mustache_test_callback(mustache_section_s *section,
  ...
  case CB_ON_TEST: {
    ...
    if (txt)
      ...
      fprintf(stderr,
              "* mustache callback for section testing detected (no "
  ...
  if (callback_expected[callback_count].cb_type != expected) {
    fprintf(stderr,
            ...
            "got %u)\n",
            callback_count,
            callback_expected[callback_count].cb_type,
            expected);
    exit(-1);
  }
  if (callback_expected[callback_count].udata1 != section->udata1) {
    fprintf(stderr,
            ...
            "%p, got %p)\n",
            callback_count,
            callback_expected[callback_count].udata1,
            section->udata1);
    exit(-1);
  }
  ++callback_count;
...
static inline void
save2file(char const *filename, char const *data, size_t length) {
...
void mustache_test(void) {
  ...
  if (!m) {
    ...
    unlink(partial_name);
    FIO_ASSERT(
        m, "Mustache template loading from file failed with error %u\n", err);
  }
  mustache_free(m);
  m = mustache_load(.data = partial2,
                    .data_len = strlen(partial2),
                    .err = &err);
  if (!m) {
    unlink(template_name);
    unlink(partial_name);
    FIO_ASSERT(
        m,
        "Mustache template loading partial as data failed with error %u\n",
        err);
  }
  mustache_free(m);
  m = mustache_load(.filename = template_name,
                    .data = template,
                    .data_len = strlen(template),
                    .err = &err);
  unlink(template_name);
  unlink(partial_name);
  ...
  FIO_ASSERT(
      !err, "Mustache template building template failed with error %u\n", err);
  FIO_ASSERT(callback_count + 1 == callback_max,
             "Callback count error %zu != %zu",
             callback_count + 1,
             callback_max);
