 ...
 * Out of bounds requests fail silently.
 */
__unused static void bitmap_set_cluster(void* bitmap_ptr,
                                        size_t offset,
                                        size_t length) {
  if (length == 0)
    return;
 ...
 *
 */
__unused static inline ssize_t bitmap_seek_empty_cluster(void* bitmap_ptr,
                                                         size_t offset,
                                                         size_t length) {
  if (length <= 1)
 ...
 *
 */
__unused static inline ssize_t bitmap_seek_set_cluster(void* bitmap_ptr,
                                                       size_t offset,
                                                       size_t length) {
  if (length == 1)
...
__unused static void bitmap_test(void) {
  ...
  if (bitmap.bitmap[0] == (((1UL << 3) - 1) << 3))
    ...
    fprintf(stderr,
            "* bitmap bitmap_set_cluster no overflow FAILED"
  ...
  if (bitmap_seek_set(&bitmap, 0) == -1 && bitmap_total_set(&bitmap, 0) == 0 &&
      (bitmap.bitmap[4] | bitmap.bitmap[5] | bitmap.bitmap[7]) == 0)
    fprintf(stderr,
            "* bitmap seek and unset single bit X 32768 bits "
  ...
  if (bitmap_seek_unset(&bitmap, 0) == 32767 &&
      ...
      bitmap.bitmap[7] == (~0 >> 1))
    fprintf(stderr,
            "* bitmap seek and set 7 bit cluster X 32768 bits "
            "passed. CPU cycles %lu\n",
            end - start);
  else
    fprintf(stderr,
  ...
  if (bitmap_seek_set(&bitmap, 0) == -1 && bitmap_total_set(&bitmap, 0) == 0 &&
      (bitmap.bitmap[4] | bitmap.bitmap[5] | bitmap.bitmap[7]) == 0)
    fprintf(stderr,
            "* bitmap seek and unset 7 bit cluster "
