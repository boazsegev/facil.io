...
void initialize_cli(int argc, char const *argv[]) {
  ...
  fio_cli_start(
      ...
      FIO_CLI_STRING("-db_host address to connect db"),
      FIO_CLI_INT("-db_port port to connect db"),
      FIO_CLI_STRING("-db_user db user to connect"),
      FIO_CLI_STRING("-db_name the name of the database to connect"),
      FIO_CLI_STRING("-db_pass the db user's password to connect")
    );
