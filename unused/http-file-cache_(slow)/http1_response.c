...
This variation is meant to be used for file caching systems, such as the
`fio_file_cache` service.

If the connection was already closed, the function will return -1. On success,
the function returns 0.
*/
static void http1_mock_pfd_close(int *pfd) { (void)pfd; }
int http1_response_send_cached_file(http_response_s *rs, int *pfd,
                                    void (*pfd_close)(int *), off_t offset,
                                    size_t length) {
  if (!pfd)
    return -1;
  if (!pfd_close)
    pfd_close = http1_mock_pfd_close;
  if (!sock_isvalid(rs->fd) || !length) {
    pfd_close(pfd);
    return -1;
  }
  ...
  if (!rs->headers_sent) {
    ...
    tmp = (length + rs1->buffer_end >= HTTP1_MAX_HEADER_SIZE)
              ? HTTP1_MAX_HEADER_SIZE - rs1->buffer_end
              : length;
    tmp = pread(*pfd, rs1->buffer + rs1->buffer_end, tmp, offset);
    if (tmp <= 0) {
      pfd_close(pfd);
      return -1;
    }
  ...
  if (length)
    return (sock_write2(.uuid = rs->fd, .buffer = pfd, .length = length,
                        .offset = offset, .is_pfd = 1, .move = 1) >= 0);
  else
    pfd_close(pfd);
